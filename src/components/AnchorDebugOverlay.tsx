...
export function AnchorDebugOverlay({ anchors, isVisible, onToggle }: AnchorDebugOverlayProps) {
  if (!isVisible) {
    return (
      <button
        onClick={onToggle}
        className="fixed bottom-2 right-2 z-[100] px-2 py-1 text-xs bg-black/70 text-white rounded hover:bg-black/90"
      >
        Debug 📍
      </button>
    );
  ...
  return (
    <>
      ...
      <button
        onClick={onToggle}
        className="fixed bottom-2 right-2 z-[100] px-3 py-1 text-xs bg-red-600 text-white rounded hover:bg-red-700"
      >
        Close Debug ✕
      </button>
    </>
